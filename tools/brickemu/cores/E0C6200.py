...
class E0C6200():
    def __init__(self, mask, clock, interconnect):
        ...
        self._halt_cycles = 0    # prof: emulated cycles spent NOT retiring an instruction (HALT/RESET)
        self._fast_cycles = 0    # prof: emulated cycles retired while on the fast (main) clock (CLKCHG set)
        self._pc_prof_on = False # prof: when True, histogram fetch PC + op-class of fast-clock instructions
        self._pc_hist = {}
        self._op_hist = {}       # prof: fast-clock instruction-handler (op class) -> count
    ...
    def halt_cycles(self):    # prof: emulated cycles spent idle (HALT/RESET, no instruction retired)
        return self._halt_cycles

    def fast_cycles(self):    # prof: emulated cycles retired on the fast/main clock (CLKCHG set)
        return self._fast_cycles

    def enable_pc_prof(self):    # prof: start recording fast-clock fetch-PC histogram
        self._pc_prof_on = True

    def pc_hist(self):
        return self._pc_hist

    def op_hist(self):
        return self._op_hist

    def reset_pc_hist(self):
        self._pc_hist = {}
        self._op_hist = {}
    ...
    def clock(self):
        ...
        if (not self._RESET):
            if (not self._HALT):
                ...
                self._if_delay = False
                pc = self._PC
                opcode = self._ROM.get_word(pc * 2)
                handler = self._execute[opcode]
                if (self._pc_prof_on and (self._CTRL_OSC & IO_CLKCHG)):
                    self._pc_hist[pc] = self._pc_hist.get(pc, 0) + 1
                    name = handler.__name__
                    self._op_hist[name] = self._op_hist.get(name, 0) + 1
                exec_cycles = handler(self, opcode)
            ...
            if (not (self._CTRL_OSC & IO_CLKCHG)):
                ...
                self._fast_cycles += exec_cycles    # prof: cycles retiring on the 1.06MHz main clock
